...
const migrateConfig = {
  ...
  '209': (state: RootState) => {
    try {
      // Clear default cherryai models for custom build
      const EMPTY_MODEL = { id: '', name: '', provider: '', group: '' }
      if (state.llm.defaultModel?.provider === 'cherryai') {
        state.llm.defaultModel = EMPTY_MODEL
      }
      if (state.llm.quickModel?.provider === 'cherryai') {
        state.llm.quickModel = EMPTY_MODEL
      }
      if (state.llm.translateModel?.provider === 'cherryai') {
        state.llm.translateModel = EMPTY_MODEL
      }
      state.assistants.assistants.forEach((assistant) => {
        if (assistant.model?.provider === 'cherryai') {
          assistant.model = EMPTY_MODEL
        }
        if (assistant.defaultModel?.provider === 'cherryai') {
          assistant.defaultModel = EMPTY_MODEL
        }
      })
  ...
  '210': (state: RootState) => {
    try {
      const EMPTY_MODEL = { id: '', name: '', provider: '', group: '' }
      // Clean up residual models from old providers (cherryai, dashscope, cherryin)
      const residualProviders = ['cherryai', 'dashscope', 'cherryin']
      const isResidual = (model?: { provider?: string }) => {
        return !!model && residualProviders.includes(model.provider || '')
      }
      if (isResidual(state.llm.defaultModel)) {
        state.llm.defaultModel = EMPTY_MODEL
      }
      if (isResidual(state.llm.quickModel)) {
        state.llm.quickModel = EMPTY_MODEL
      }
      if (isResidual(state.llm.translateModel)) {
        state.llm.translateModel = EMPTY_MODEL
      }
      if (isResidual(state.llm.topicNamingModel)) {
        state.llm.topicNamingModel = EMPTY_MODEL
      }
      state.assistants.assistants.forEach((assistant) => {
        if (isResidual(assistant.model)) {
          assistant.model = EMPTY_MODEL
        }
        if (isResidual(assistant.defaultModel)) {
          assistant.defaultModel = EMPTY_MODEL
        }
      })
  ...
  '211': (state: RootState) => {
    try {
      // Force top navigation for all users
      state.settings.navbarPosition = "top"
      // Add 'store' to visible sidebar icons if not present
      if (state.settings.sidebarIcons && !state.settings.sidebarIcons.visible.includes('store')) {
        state.settings.sidebarIcons.visible = ['assistants', 'store']
      }
  ...
  '212': (state: RootState) => {
    try {
      // Force theme to follow system
      state.settings.theme = 'system' as any
  ...
  '213': (state: RootState) => {
    try {
      if (state.settings?.sidebarIcons?.visible) {
        state.settings.sidebarIcons.visible = state.settings.sidebarIcons.visible.filter((icon: string) => icon !== 'agents')
      }
      if (Array.isArray(state.tabs?.tabs)) {
