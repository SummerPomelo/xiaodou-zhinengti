...
import settings, { setNavbarPosition } from './settings'
...
export const persistor = persistStore(store, undefined, () => {
  // Force top navigation after persisted state is rehydrated
  const state = store.getState()
  if (state.settings.navbarPosition !== 'top') {
    store.dispatch(setNavbarPosition('top'))
    logger.info('Forced navbarPosition to top after rehydration')
  }

  // Initialize notes path after rehydration if empty
  // reuse state from above
