...
export const autoRenameTopic = async (assistant: Assistant, topicId: string) => {
  ...
  try {
    ...
    // Skip LLM topic naming if quickModel is not configured (custom build)
    const quickModel = store.getState().llm.quickModel
    if (!quickModel || !quickModel.id || !quickModel.provider) {
