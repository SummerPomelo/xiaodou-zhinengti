...
export function useAppInit() {
  ...
  useEffect(() => {
    if (privacyPolicyVersion === LATEST_PRIVACY_POLICY_VERSION) {
      ...
      // Skip privacy policy notice for custom build
  ...
  useEffect(() => {
    try {
      if (proxyMode === 'system') {
        window.api.setProxy('system', undefined).catch((e) => logger.warn('Failed to set proxy', e))
      } else if (proxyMode === 'custom') {
        void (proxyUrl && window.api.setProxy(proxyUrl, proxyBypassRules))
      } else {
        window.api.setProxy('', undefined).catch((e) => logger.warn('Failed to set proxy', e))
      }
    } catch (e) {
      logger.warn('Failed to set proxy', e as Error)
    }
